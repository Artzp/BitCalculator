...
export const TaskAssignmentInterface: React.FC<TaskAssignmentInterfaceProps> = ({ 
  ...
  const calculatePlayerWorkloads = (tasks: any[], members: any[]) => {
    const workloads: PlayerWorkload[] = members.map(member => {
      const playerId = member.collaboration?.userId || member.user?.id || member.id;
  ...
  const reloadTaskData = async () => {
    ...
    try {
      ...
      if (userSettlements.length > 0) {
        ...
        // Data is now passed as props, no need to reload
      }
    } catch (error) {
