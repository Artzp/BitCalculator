...
export class EnhancedFirebaseService {
  ...
  async loadCollaborativeProjectData(collaboration: ProjectCollaboration): Promise<{ project: any; tasks: any[] } | null> {
    try {
      ...
      const projects = Array.isArray(ownerData.settlement.projects) ? ownerData.settlement.projects : [];
      
      // Find the actual project in the owner's settlement
      const actualProject = projects.find(p => p.id === collaboration.projectId);
      
      if (!actualProject) {
        console.log('❌ Original project not found in owner settlement', {
          searchingFor: collaboration.projectId,
          availableProjects: projects.map(p => ({ id: p.id, name: p.name }))
        });
        return null;
      }

      // Ensure tasks is an array and find tasks associated with this project
      const tasks = Array.isArray(ownerData.settlement.tasks) ? ownerData.settlement.tasks : [];
      const projectTasks = tasks.filter(t => t.projectId === collaboration.projectId);

      console.log('✅ Collaborative project data loaded:', {
        projectName: actualProject.name,
        itemCount: actualProject.items?.length || 0,
        taskCount: projectTasks.length,
        tasksDataType: typeof ownerData.settlement.tasks,
        tasksIsArray: Array.isArray(ownerData.settlement.tasks)
      });
